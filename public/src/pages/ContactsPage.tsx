...
export const ContactsPage: React.FC<ContactsPageProps> = ({ onOpenOrder }) => {
  ...
  const [quickWishes, setQuickWishes] = useState('');
  ...
  const handleQuickSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      onOpenOrder(
        `Быстрый вопрос со страницы контактов от ${quickName || 'Клиента'} (${quickPhone})${
              ...
              <form onSubmit={handleQuickSubmit} className="space-y-3">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    value={quickWishes}
                    onChange={(e) => setQuickWishes(e.target.value)}
                    className="w-full bg-slate-50 border border-slate-300 rounded-xl px-3.5 py-2.5 text-xs sm:text-sm text-slate-800 placeholder-slate-400 focus:outline-hidden focus:border-orange-500 resize-none"
                  />
                </div>
                <button
                  type="submit"
