...
export const GalleryPage: React.FC<GalleryPageProps> = ({ onOpenOrder }) => {
  ...
  const allItems = useMemo(() => {
    const items = [...REAL_PORTFOLIO, ...customItems] as GalleryItem[];
    if (!portfolioOrder.length) return items;
    const rank = new Map(portfolioOrder.map((id, index) => [id, index]));
    return [...items].sort((a, b) =>
      (rank.get(a.id) ?? Number.MAX_SAFE_INTEGER) - (rank.get(b.id) ?? Number.MAX_SAFE_INTEGER)
    );
  }, [customItems, portfolioOrder]);
  ...
  const moveItem = (sourceId: string, targetId: string) => {
    ...
    notify('Новый порядок карточек сохранён');
  };
          ...
          {filtered.map((item) => (
            <div
              ...
              onDrop={(e) => {
                if (!editMode) return;
                e.preventDefault();
                moveItem(e.dataTransfer.getData('text/plain') || draggedId || '', item.id);
                setDraggedId(null);
              }}
              onDragEnd={() => setDraggedId(null)}
              className={`white-card rounded-3xl overflow-hidden flex flex-col justify-between group relative ${
                editMode ? `cursor-grab active:cursor-grabbing ${draggedId === item.id ? 'opacity-50 ring-2 ring-orange-500' : ''}` : 'cursor-pointer'
              }`}
            >
              {editMode && (
