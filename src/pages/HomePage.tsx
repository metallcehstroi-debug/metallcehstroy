...
export const HomePage: React.FC<HomePageProps> = ({
  ...
  const recentWorks = REAL_PORTFOLIO.slice(0, 9);
        ...
        <div className="max-w-[1680px] mx-auto px-4 sm:px-6 2xl:px-10">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
            {recentWorks.map((item) => (
              <div
                key={item.id}
                onClick={() => setAlbumItem(item)}
                className="white-card rounded-3xl overflow-hidden cursor-pointer flex flex-col justify-between group"
              >
